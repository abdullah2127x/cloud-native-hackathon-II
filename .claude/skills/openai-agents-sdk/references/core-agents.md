...
## Agent Cloning (Dynamic Instructions)

```python
# Clone with modified instructions per-request
personalized = agent.clone(
    instructions=f"You are helping {user.name}. Their plan is {user.plan}.",
)
result = await Runner.run(personalized, user_message)
```
